...
void CTestView::OnDrawpic() 
{
	...
	CInputDlg dlg;
	if(IDOK==dlg.DoModal()){   //调用对话框模块，判断是否单机OK按钮
		n = dlg.m_n;           //等分点个数
		r = dlg.m_r;           //r为圆的半径
	}else return;
	RedrawWindow();   //重绘窗口
	P = new CP2[n];   //动态创建一位数组
	Diamond();        //调用绘制金刚石图案函数
	delete []P;       //释放数组内存空间
...
void CTestView::Diamond()
{
	...
	NewPen.CreatePen(PS_SOLID,1,RGB(0,255,0));   //创建蓝色画笔
	pOldPen = pDC->SelectObject(&NewPen);  //将蓝色画笔选入设备上下文

	double Alpha,Theta;        //定义金刚石图案的起始角和等分角
	Theta = 2*PI/n;            //等分角
